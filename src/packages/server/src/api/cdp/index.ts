...
function isRetryableError(error: any): boolean {
  ...
  const causeMessage =
    error?.cause?.message || error?.originalError?.message || "";
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (isRetryableError(error) && attempt < maxRetries) {
        ...
        logger.warn(
          `[CDP API] DB connection error, retrying in ${delay}ms (attempt ${attempt + 1}/${maxRetries})...`,
        );
      ...
      if (isRetryableError(error)) {
        logger.warn(
          "[CDP API] Pool dead after retries, attempting fresh direct connection...",
        );
        try {
          ...
          logger.error(
            "[CDP API] Fresh connection also failed:",
            freshError?.message,
          );
          throw freshError;
        }
...
async function resolveWalletAccountName(
  ...
  if (!dbExecute) {
    logger.warn(
      `[${logPrefix}] Database not available, falling back to entityId`,
    );
    return entityId;
  }
  ...
  try {
    ...
    logger.warn(
      `[${logPrefix}] No user_registry entry for entity_id=${entityId.substring(0, 8)}..., using entityId`,
    );
    return entityId;
  } catch (error: unknown) {
    logger.error(
...
export function cdpRouter(serverInstance: AgentServer): express.Router {
   ...
   */
  async function getWalletAddressFromEntity(
    userId: string,
  ): Promise<string | null> {
    if (!dbAdapter) {
      logger.warn(
        "[CDP API] Database not available, cannot fetch entity metadata",
      );
      return null;
    }
    ...
    try {
      ...
      const walletAddress = entity.metadata?.walletAddress as
        | string
        | undefined;
      ...
      if (
        ...
        logger.debug(
          `[CDP API] Found wallet address in entity metadata: ${walletAddress}`,
        );
      ...
      logger.warn(
        "[CDP API] Error fetching entity metadata:",
        error instanceof Error ? error.message : String(error),
      );
      return null;
    }
    ...
    try {
      ...
      if (chain && !MAINNET_NETWORKS.includes(chain as any)) {
        return sendError(
          ...
          400,
          "INVALID_CHAIN",
          `Invalid or unsupported chain: ${chain}`,
        );
      }
      ...
      const result = await cdpTransactionManager.getTokenBalances(
        accountName,
        chain,
        false,
  ...
  router.get("/wallet/nfts", async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const result = await cdpTransactionManager.getNFTs(
        accountName,
        chain,
        false,
        walletAddress || undefined,
  ...
  router.get("/wallet/history", async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const result = await cdpTransactionManager.getTransactionHistory(
        accountName,
        walletAddress || undefined,
      );
    ...
    try {
      ...
      if (!network || !to || !token || !amount) {
        return sendError(
          ...
          400,
          "INVALID_REQUEST",
          "Missing required fields: network, to, token, amount",
        );
      }
      ...
      const detailsMatch = rawErrorMessage.match(
        /Details:\s*(.+?)(?:\nVersion:|$)/s,
      );
      if (detailsMatch) {
    ...
    try {
      ...
      if (!network || !to || !contractAddress || !tokenId) {
        return sendError(
          ...
          400,
          "INVALID_REQUEST",
          "Missing required fields: network, to, contractAddress, tokenId",
        );
      }
  ...
  async function resolveTokenToAddress(
    ...
    if (trimmedToken.toLowerCase() === "eth") {
      if (network === "polygon") {
        ...
        logger.info(
          `[CDP API] Using WETH contract address for ETH on Polygon: ${wethAddress}`,
        );
        return wethAddress as `0x${string}`;
      }
      logger.info(
        `[CDP API] Using native token address for ETH: ${NATIVE_TOKEN_ADDRESS}`,
      );
    ...
    if (trimmedToken.toLowerCase() === "weth") {
      ...
      if (wethAddress) {
        logger.info(
          `[CDP API] Using WETH contract address for ${network}: ${wethAddress}`,
        );
        return wethAddress as `0x${string}`;
      }
      logger.warn(
        `[CDP API] No WETH address configured for network ${network}`,
      );
    }
    ...
    if (
      ...
      logger.info(
        `[CDP API] Using native token address for ${trimmedToken.toUpperCase()}: ${NATIVE_TOKEN_ADDRESS}`,
      );
      return NATIVE_TOKEN_ADDRESS as `0x${string}`;
    }
    ...
    if (trimmedToken.toLowerCase() === "wmatic" && network === "polygon") {
      logger.info(
        `[CDP API] Using WMATIC contract address for Polygon: ${WMATIC_ADDRESS}`,
      );
    ...
    if (trimmedToken.startsWith("0x") && trimmedToken.length === 42) {
      logger.debug(
        `[CDP API] Token ${token} looks like an address, validating via searchTokens`,
      );
      try {
        ...
        const foundToken = searchResult.tokens?.find(
          (t: any) =>
            t.contractAddress?.toLowerCase() === trimmedToken.toLowerCase() &&
            t.chain === network,
        );

        if (foundToken) {
          logger.info(
            `[CDP API] Validated address ${token} exists: ${foundToken.symbol} (${foundToken.name})`,
          );
          return trimmedToken as `0x${string}`;
        }
        logger.warn(
          `[CDP API] Address ${token} not found via searchTokens for network ${network} - may be fake/invalid`,
        );
      } catch (error) {
        logger.warn(
          `[CDP API] Failed to validate address ${token}:`,
          error instanceof Error ? error.message : String(error),
        );
      }
    ...
    logger.debug(
      `[CDP API] Resolving token symbol from searchTokens for ${trimmedToken}`,
    );
    try {
      const searchResult = await cdpTransactionManager.searchTokens({
        query: trimmedToken,
      ...
      const matchedToken = searchResult.tokens?.find(
        (t: any) =>
          t.symbol?.toLowerCase() === trimmedToken.toLowerCase() &&
          t.chain === network &&
          t.contractAddress,
      );
      ...
      if (matchedToken?.contractAddress) {
        logger.info(
          `[CDP API] Resolved ${token} to ${matchedToken.contractAddress} via searchTokens`,
        );
        return matchedToken.contractAddress.toLowerCase() as `0x${string}`;
      }
    } catch (error) {
      logger.warn(
        `[CDP API] Failed to resolve token symbol ${token}:`,
        error instanceof Error ? error.message : String(error),
      );
    }
    ...
    try {
      ...
      if (!network || !fromToken || !toToken || !fromAmount) {
        return sendError(
          ...
          400,
          "INVALID_REQUEST",
          "Missing required fields: network, fromToken, toToken, fromAmount",
        );
      }

      // Resolve token symbols/addresses to proper addresses (same logic as action handler)
      logger.debug(
        `[CDP API] Resolving tokens for swap price: ${fromToken} -> ${toToken} on ${network}`,
      );
      ...
      if (!resolvedFromToken) {
        return sendError(
          ...
          `Could not resolve source token: ${fromToken}`,
        );
      }
      if (!resolvedToToken) {
        return sendError(
          ...
          400,
          "TOKEN_RESOLUTION_FAILED",
          `Could not resolve destination token: ${toToken}`,
        );
      }

      logger.debug(
        `[CDP API] Resolved tokens: ${resolvedFromToken} -> ${resolvedToToken}`,
      );
    ...
    try {
      ...
      if (
        ...
        return sendError(
          ...
          400,
          "INVALID_REQUEST",
          "Missing required fields: network, fromToken, toToken, fromAmount, slippageBps",
        );
      }

      // Resolve token symbols/addresses to proper addresses (same logic as action handler)
      logger.debug(
        `[CDP API] Resolving tokens for swap: ${fromToken} -> ${toToken} on ${network}`,
      );
