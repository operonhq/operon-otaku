  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (isRetryableError(error)) {
        logger.warn(
          "[Auth] Pool dead after retries, attempting fresh direct connection...",
        );
        try {
          ...
          logger.error(
            "[Auth] Fresh connection also failed:",
            freshError?.message,
          );
          throw freshError;
        }
...
async function initUserRegistry(db: any): Promise<void> {
  ...
  if (!warmupSuccess) {
    ...
    try {
      ...
      logger.info(
        "[Auth] Database reachable via fresh connection (pool may need time)",
      );
    } catch (freshError: any) {
      logger.error(
        "[Auth] Fresh connection also failed - database may be unreachable:",
        freshError?.message,
      );
    }
  }
...
export function createAuthRouter(serverInstance: AgentServer): express.Router {
  ...
  async function verifyUserBinding(
    ...
    if (!db) {
      ...
      logger.warn(
        "[Auth] Database not available, using fallback (development mode)",
      );
      const fallbackId = crypto
        .createHash("sha256")
        .update(`${cdpUserId}:${normalizedEmail}`)
        .digest("hex")
        .substring(0, 32);
