...
export function createAgentMemoryRouter(
  ...
  async function verifyMemoryOwnership(
    ...
    try {
      ...
      logger.warn(
        `[MEMORY ACCESS] User ${userId.substring(0, 8)}... denied access to memory ${memoryId}`,
      );
      return { owned: false, memory };
    } catch (error: unknown) {
      logger.error(
        "[MEMORY ACCESS] Error checking memory ownership:",
  ...
  router.patch(
    ...
    async (req: AuthenticatedRequest, res) => {
      ...
      try {
        ...
        logger.success(
          `[MEMORY UPDATE] Successfully updated memory ${memoryId}`,
        );
        sendSuccess(res, {
          id: memoryId,
  ...
  router.delete(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (!hasUnrestrictedAccess(req)) {
          logger.warn(
            `[DELETE ALL MEMORIES] User ${req.userId?.substring(0, 8)}... attempted to clear all agent memories`,
          );
          return sendError(
            res,
            403,
            "FORBIDDEN",
            "Only administrators can clear all agent memories",
          );
        }
        ...
        logger.info(
          `[DELETE ALL MEMORIES] Admin/server cleared ${deleted} memories for agent ${agentId}`,
        );
        sendSuccess(res, {
          deleted,
  ...
  router.delete(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (!hasUnrestrictedAccess(req)) {
          logger.warn(
            `[DELETE ALL ROOM MEMORIES] User ${req.userId?.substring(0, 8)}... attempted to clear all room memories`,
          );
          return sendError(
            res,
            403,
            "FORBIDDEN",
            "Only administrators can clear all room memories",
          );
        }
        ...
        logger.info(
          `[DELETE ALL ROOM MEMORIES] Admin/server cleared memories for room ${roomId}`,
        );
        res.status(204).send();
      } catch (error) {
        logger.error(
