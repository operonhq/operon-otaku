...
export const webFetch: Action = {
  ...
  parameters: {
    ...
    formats: {
      type: "array",
      description:
        "Array of formats to return: 'markdown', 'html', 'rawHtml', 'screenshot', 'links'. Defaults to ['markdown', 'html']",
      required: false,
    },
    onlyMainContent: {
      type: "boolean",
      description:
        "Whether to extract only main content (removes headers, footers, nav). Defaults to true",
      required: false,
    },
  },
  ...
  handler: async (
    ...
    try {
      const firecrawlService =
        runtime.getService<FirecrawlService>("FIRECRAWL");
      if (!firecrawlService) {
      ...
      if (!url) {
        const errorMsg =
          "Missing required parameter 'url'. Please specify the URL to fetch.";
