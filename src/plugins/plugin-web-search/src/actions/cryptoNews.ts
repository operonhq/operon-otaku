...
export const cryptoNews: Action = {
  ...
  parameters: {
    query: {
      type: "string",
      description:
        "The crypto/blockchain/DeFi news query (e.g., 'Aave', 'Ethereum merge', 'DeFi hacks')",
      required: true,
    },
    source: {
      type: "string",
      description:
        "Specific crypto news source: 'theblock', 'coindesk', 'decrypt', 'dlnews', 'coinbureau', 'cointelegraph', 'blockworks'. Leave empty to search all sources.",
      required: false,
    },
    categories: {
      type: "array",
      description:
        "CoinDesk categories to filter by: 'markets', 'tech', 'policy', 'defi', 'nft', 'layer-2', 'regulation'. Only applies when using CoinDesk API.",
      required: false,
    },
    time_range: {
      type: "string",
      description:
        "Time range filter: 'day', 'week', 'month', 'year' (or 'd', 'w', 'm', 'y'). Defaults to 'week' for recent news.",
      required: false,
    },
    max_results: {
      type: "number",
      description:
        "Maximum number of results (1-100 for CoinDesk API, 1-20 for Tavily). Defaults to 10.",
      required: false,
    },
    search_depth: {
      type: "string",
      description:
        "Search depth: 'basic' or 'advanced'. Only applies to Tavily fallback. Defaults to 'basic'.",
      required: false,
    },
    include_body: {
      type: "boolean",
      description:
        "Include full article body in response (CoinDesk API only). Defaults to false for performance.",
      required: false,
    },
  },
  ...
  validate: async (
    ...
    try {
      ...
      const coindeskService =
        runtime.getService<CoinDeskService>("COINDESK_NEWS");
  ...
  handler: async (
    ...
    try {
      ...
      if (!query) {
        const errorMsg =
          "Missing required parameter 'query'. Please specify what crypto news to search for.";
      ...
      const maxResults = params?.max_results
        ? Math.min(Math.max(1, params.max_results), 100)
        : 10;
      const searchDepth =
      ...
      if (
        coindeskService &&
        coindeskService.isConfigured() &&
        (!sourceKey || sourceKey === "coindesk")
      ) {
        ...
        if (
          coindeskResponse.success &&
          coindeskResponse.data?.articles.length
        ) {
          ...
          responseText += articles
            .map((article, index) => {
              ...
              if (article.authors?.length)
                parts.push(`*By: ${article.authors.join(", ")}*`);
              if (article.publishedAt) {
                const pubDate = new Date(article.publishedAt);
                parts.push(
                  `*Published: ${pubDate.toLocaleDateString()} at ${pubDate.toLocaleTimeString()}*`,
                );
              }
        ...
        logger.warn(
          `[CRYPTO_NEWS] CoinDesk API returned no results, falling back to Tavily`,
        );
      }
      ...
      const sourceDomain =
        sourceKey && sourceKey in CRYPTO_NEWS_SOURCES
      ...
      if (sourceDomain) {
        ...
        logger.info(
          `[CRYPTO_NEWS] Using Tavily with source filter: ${sourceKey}`,
        );
      } else {
