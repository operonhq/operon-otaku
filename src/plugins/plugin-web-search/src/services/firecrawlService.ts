...
export class FirecrawlService extends Service {
  ...
  capabilityDescription =
    "Fetch and scrape webpage content using Firecrawl API";
  ...
  async initialize(runtime: IAgentRuntime): Promise<void> {
    this.apiKey = String(runtime.getSetting("FIRECRAWL_API_KEY") ?? "");

    if (!this.apiKey) {
      logger.warn(
        "FIRECRAWL_API_KEY not configured. WEB_FETCH action will not be available.",
      );
    } else {
