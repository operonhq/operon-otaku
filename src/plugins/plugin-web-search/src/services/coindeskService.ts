...
export class CoinDeskService extends Service {
  ...
  capabilityDescription =
    "Fetch cryptocurrency news articles from CoinDesk API";
  ...
  async initialize(runtime: IAgentRuntime): Promise<void> {
    this.apiKey = String(runtime.getSetting("COINDESK_API_KEY") ?? "");

    if (!this.apiKey) {
      logger.warn(
        "COINDESK_API_KEY not configured. CRYPTO_NEWS will fall back to Tavily.",
      );
    } else {
  ...
  async searchNews(
    ...
    try {
      ...
      if (options.authors?.length)
        params.append("authors", options.authors.join(","));

      // Date filtering - support multiple formats
      if (options.startDate) params.append("start_date", options.startDate);
      if (options.endDate) params.append("end_date", options.endDate);
      if (options.publishedAfter)
        params.append("published_after", options.publishedAfter);
      if (options.publishedBefore)
        params.append("published_before", options.publishedBefore);
      ...
      logger.info(
        `[CoinDesk] Fetching news: ${options.query || "latest"} (limit: ${limit})`,
      );
      ...
      logger.info(
        `[CoinDesk] Found ${articles.length} articles (total: ${total})`,
      );
